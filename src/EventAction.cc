...
EventAction::EventAction(DetectorConstruction* det): detector(det), fastCollID(-1), bottomBGOCollID(-1), SASCollID(-1)
{
  printModulo = 10000;
}
...
void EventAction::WriteToROOT(const G4Event* evt)
{
  ...
  if ((fastEvent.nhits > 0)||(bottomBGOEvent.nhits > 0)) G4dataTree->Fill();
}
