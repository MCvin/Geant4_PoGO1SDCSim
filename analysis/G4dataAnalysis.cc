...
//User libs
#include "G4data.h"
#include "inputs/Integrator.h"

using namespace std;

// user settings
bool veto_vhdl = false;
bool veto_perfect = false;
float hitThreshold = 10;
float trigThreshold = 10;
float upperD = 3000; //2500;
...
// R = 0.1, lambda = 5, PeakEnergy = 52
// R = 0., lambda = 10, PeakEnergy = 55.5
// PeakEnergy = 56.5
//~ float R = 0.1;
//~ float lambda = 5.;
...
void displayHistos()
{
  ...
  //TFile* f_Calib = new TFile("Calibrations/PureFastSpectrum_rmAfterPulse_ProperPileUp.root","READ");
  //TFile* f_Calib = new TFile("Calibrations/PureSlowSpectrum_rmAfterPulse.root","READ");
  TFile* f_Calib = new TFile("Calibrations/SDCSpectrum_rmAfterPulse.root","READ");
  hist_Calib = (TH1F*) f_Calib->Get("hist_peak_subtr");
  hist_Calib->Scale(S1->Integral()/hist_Calib->Integral());
  hist_Calib->SetLineColor(kRed);
  //hist_Calib->Rebin(16);
  hist_Calib->Draw("sames");
...
{
  //ifstream PAP_EJ204("inputs/EJ204_PAP_pure.dat"); if (!PAP_EJ204.is_open()) cout << "Unable to open file" << endl;
  ifstream PAP_EJ204("inputs/EJ204_PAP.dat"); if (!PAP_EJ204.is_open()) cout << "Unable to open file" << endl;
  //ifstream PAP_BGO("inputs/BGO_PAP_pure.dat"); if (!PAP_BGO.is_open()) cout << "Unable to open file" << endl;
  ifstream PAP_BGO("inputs/BGO_PAP.dat"); if (!PAP_BGO.is_open()) cout << "Unable to open file" << endl;
  ifstream SPEP("inputs/PMT_SPEP.dat"); if (!SPEP.is_open()) cout << "Unable to open file" << endl;
  ifstream SPEP_sigma("inputs/PMT_SPEP_sigma.dat"); if (!SPEP_sigma.is_open()) cout << "Unable to open file" << endl;
  ...
  for (int i = 0; i < 91; i++) {
    SPEP >> nADC_PhE[i];
    SPEP_sigma >> nADC_PhE_sigma[i];
    PAP_EJ204 >> PAP_temp;
    nPhE_keV_EJ204[i] = PAP_temp / nADC_PhE[i] / NonLinearity_EJ204(PeakEnergy);
    PAP_BGO >> PAP_temp;
    nPhE_keV_BGO[i] = PAP_temp / nADC_PhE[i] / NonLinearity_BGO(PeakEnergy);
  }
  PAP_EJ204.close();
  PAP_BGO.close();
  SPEP.close();
  ...
  //~ //f_SPE = new TF1("f_SPE","[0]*1/[1]*TMath::Exp(-x/[1])+(1-[0])*1/[2]/TMath::Sqrt(2*TMath::Pi())*TMath::Exp(-TMath::Power((x-[3])/[2],2)/2)",0,100);
  //~ f_SPE = new TF1("f_SPE","[0]*1/[1]*TMath::Exp(-x/[1]-TMath::Power(3*x/[3],2)/2)+(1-[0])*1/[2]/TMath::Sqrt(2*TMath::Pi())*TMath::Exp(-TMath::Power((x-[3])/[2],2)/2)",0,100);
  //~ f_SPE->SetParameters(R,lambda,nADC_PhE_sigma[0],nADC_PhE[0]);
  //~ nADC_PhE[0] = Integrator::GetMean(f_SPE,0,nADC_PhE[0]+nADC_PhE_sigma[0]*8);
}
...
void EnergyConversion_keV2ADC(Processed_data *G4Pro)
{
  ...
  for (int x = 0; x < G4Pro->PMT_fast->nUnits; x++) {
    ...
    //ADC_fast = nPhE_fast * ADCperPE;
    // New way of simulating a PMT
    //~ ADC_fast = 0;
    //~ for (int nPE = 0; nPE < nPhE_fast; nPE++){
      //~ ADC_fast += f_SPE->GetRandom(0,100);
    //~ }
